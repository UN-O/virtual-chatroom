...
| UI Shell | ✅ 88% | TimeBar 雙軌重構完成（劇情進度 + 倒數計時）；CharacterAvatar 元件完成並已替換所有頭像渲染點；貼圖/emoji、online status 仍未完成 |
...
| 程式碼結構（v3 對齊）| 🔲 85% | CharacterAvatar 元件完成；Claude Sonnet provider 完成；缺 memory.ts 模組 |
