...
## P_DESIGN — 設計維修（2026-03-13）

> 依據 `files/DESIGN.md` 對所有 chat UI 元件做全面設計系統對齊。

### 已完成修復

- [x] **`page.tsx` 硬編碼顏色全面換 token**
  - `bg-gray-50` → `bg-background`；`text-blue-900` → `text-foreground`；`bg-blue-600` → `bg-primary`；`text-gray-*` → `text-muted-foreground`/`text-foreground`；`border-gray-200` → `border-border`；`bg-white` → `bg-card`
  - 刪除 session card 的 `text-gray-800`/`text-gray-500`/`text-gray-400` 硬編碼
  - 刪除 developer tools panel 的 `bg-gray-100`/`border-gray-200`/`bg-gray-50` 硬編碼
  - 替換 inline SVG 垃圾桶圖示為 `<Trash2>` from `lucide-react`

- [x] **`game/[sessionId]/page.tsx` loading/error 狀態換 token**
  - Loading state：`bg-gray-100 text-gray-500 "Loading Session {id}..."` → `bg-background text-muted-foreground + Clock spinner + "載入劇情中…"`（中文 copy，不洩漏 session ID）
  - Not Found state：`bg-gray-50 text-gray-800 bg-blue-600` → `bg-background text-foreground bg-primary`

- [x] **`ChatWindow.tsx` 多項修復**
  - `opacity-80` on 群組成員數 → 改用 `text-[var(--chat-header-foreground)] opacity-70`（不再用 opacity hack 當顏色）
  - 硬編碼 `3 位成員` → 動態計算：從 `groups` 找 group，`group.members.length + 1`
  - Sender name `text-xs text-muted-foreground` → 加 `font-medium`，與 timestamp 視覺區分
  - 訊息泡泡：`rounded-2xl px-4 py-2.5` → `rounded-xl px-3.5 py-2`（更緊湊，更 LINE-like）
  - 尾角：`rounded-br-md`/`rounded-bl-md` → `rounded-br-[4px]`/`rounded-bl-[4px]`（更細膩的 tail）
  - `MoreVertical` 按鈕加 `aria-label="更多選項"`

- [x] **`TimeBar.tsx` 多項修復**
  - 外層 div 加 `min-h-[56px]` 防止 goal chip 出現時的 layout shift
  - 虛擬時間字體：`text-lg` → `text-2xl`（更突出的主要資訊）
  - 劇情進度標籤：`text-xs` → `text-[10px]`（降低視覺重量）
  - 階段計時器 bar 高度：`h-1.5` → `h-2.5`（提升急迫感可視性）
  - Fast-forward 按鈕：`animate-pulse` → `animate-[ff-ready_2s_ease-in-out_infinite]`（專屬 glow 動畫，不再是通用 loading pulse）
  - Fast-forward 按鈕 disabled 狀態加 `title="完成所有目標後才能快進"` tooltip
  - Debug toggle 按鈕：`title="Toggle Debug Panel"` → `aria-label="切換偵錯面板"`

- [x] **`globals.css` 全面更新**
  - Light mode：`--background` 微暖化 `0.96 0.008 250`；`--sidebar` 與 `bg` 分離 `0.985 0.004 250`；`--primary`/`--accent` 從 145° 綠 → 160° 青色（更電影感）；`--chat-bubble-self` 改為繼承 `var(--primary)`（單一來源）；`--chat-bubble-other` 加 subtle tint `0.97 0.003 250`（不再純白）
  - Dark mode：`--background` 加深 `0.12 0.012 255`；`--card`/`--sidebar` 收緊 `0.17 0.01 255`；`--chat-bubble-other` 加深 `0.22 0.015 255`；`--chat-header` 從 teal slab 改為 surface-level `0.20 0.012 255`
  - 新增 PAD glow tokens：`--pad-happy-glow`/`--pad-angry-glow`/`--pad-sad-glow`/`--pad-neutral-glow`
  - 新增 `--phase-transition-bg` token
  - `@layer base body` 加 `font-feature-settings: "kern" 1`、`-webkit-font-smoothing: antialiased`、`-moz-osx-font-smoothing: grayscale`
  - 新增 `@keyframes ff-ready`（fast-forward glow）和 `@keyframes message-in`（bubble slide-in）

### 未完成（保留為後續 P_DESIGN 項目）

- [ ] **[DESIGN-P1] 訊息泡泡進場動畫**：在每個 message bubble wrapper 加 `animate-[message-in_180ms_ease-out]`。keyframe 已定義於 globals.css，待接線至 ChatWindow。

- [ ] **[DESIGN-P1] 角色頭像 PAD glow**：在 `CharacterAvatar` 計算 expression → 對應 `--pad-*-glow` CSS variable → `style={{ boxShadow }}` 套用。PAD glow token 已在 globals.css 定義。

- [ ] **[DESIGN-P1] ChatWindow header 改用 `CharacterAvatar`**：目前 header 仍使用基礎 `<Avatar>`，不顯示 PAD 驅動表情；應替換為 `<CharacterAvatar>` 傳入 `pad` 和 `avatarExpressions`。

- [ ] **[DESIGN-P2] 貼圖泡泡區分**：sticker bubble 需要去掉背景（目前有 padding 但無 bubble 背景）、加 fade-in 動畫 `animate-[message-in_200ms_ease-out]`。

- [ ] **[DESIGN-P2] `window.confirm()` → shadcn AlertDialog**：`page.tsx` 刪除確認仍使用 `confirm()` 原生 dialog，需替換為 `AlertDialog`。

- [ ] **[DESIGN-P2] Unread badge 進場動畫**：在 `ChatList.tsx` 的 `Badge` 加 `animate-in zoom-in duration-150`。

- [ ] **[DESIGN-P2] Sidebar header 加遊戲識別**：ChatList header 加 MessageCircle icon + "Story Chat" 小字 + 當前 phase 標籤。

- [ ] **[DESIGN-P3] 結局畫面**：session completed 後應顯示專屬結局畫面，而非停留在 ChatWindow。

- [ ] **[DESIGN-P3] Phase transition overlay**：advancePhase 時全螢幕 overlay 顯示新 phase 時間（「下午 14:00」），1.5s 後淡出。
...
| UI Shell | ✅ 97% | TimeBar 雙軌重構完成；CharacterAvatar 元件完成；貼圖系統改為 emoji；design system 對齊（見 P_DESIGN）；hardcoded colors/loading states 已全面換 token |
